...
class ElasticsearchKibanaCLISearch:
    connection = None
    user_agent = "{}/{}".format(constants.TITLE, constants.VERSION)
    ...
    def msearch(
        ...
        search,
        aggs=None,
        size=constants.SEARCH_SPLIT_BUCKET_LIMIT,
        source=None,
        splits=1,
        range_keyword="range",
    ):
        ...
        if splits > 1:
            ...
            logger.info(
                'Search split into {} msearch requests using "{}" keyword with a query payload of {} bytes'.format(
                    str(splits), range_keyword, len(payload_data)
                )
            )
        else:
            logger.info(
                "Search delivered in single non-split msearch with query payload of {} bytes".format(len(payload_data))
            )
        ...
        r = requests.post(url, data=payload_data, headers=request_headers)
        ...
        for hit_index in range(0, splits):
            ...
            if value is None:
                value = 0
            elif value > constants.SEARCH_SPLIT_BUCKET_LIMIT:
                logger.warning(
                    "Search split {} has {} hit-results which exceeds the {} limit, "
                    "results truncated!".format(hit_index, value, constants.SEARCH_SPLIT_BUCKET_LIMIT)
                )
        ...
        logger.info(
            "results-available={available_count}; results-returned={returned_count}; "
    ...
    def __payload_header(self, index):
        return json.dumps({"index": index, "ignore_unavailable": True, "preference": int(round(time.time() * 1000))})

    def __payload_body(self, query_params, aggs_params, size=constants.SEARCH_SPLIT_BUCKET_LIMIT, source=None):
        if size > constants.SEARCH_SPLIT_BUCKET_LIMIT or size < 1:
        ...
        query = Q(
            ...
            if "minimum_should_match" in query_params
            else (1 if len(query_params["should"]) > 0 else None),
        ...
        payload_values = {
            ...
            "timeout": '"'
            + (
                str(query_params["timeout"])
                if "timeout" in query_params
                else "{}s".format(constants.SEARCH_TIMEOUT_SECONDS_DEFAULT)
            )
            + '"',
        }
    ...
    def __payload_range_splitter(self, ndjson_payload, range_keyword="range"):
        ...
        for range_key in payload_ranges.keys():
            range_values = dpath.util.get(
                return_payloads, "{}/{}/{}".format(range_key, payload_range_path, payload_range_path_key)
            )
