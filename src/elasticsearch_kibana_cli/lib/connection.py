...
from ..lib.internal_proxy import ElasticsearchKibanaCLIInternalProxy
...
class ElasticsearchKibanaCLIConnection:
    ...
    def __kbn_metadata(self, use_cache=True):
        ...
        if not metadata_find:
            raise ElasticsearchKibanaCLIException(
                "Unable to locate element for kbn_metadata " "within {}".format(self.client_connect_address)
            )
    ...
    def __kbn_cache_basename(self, base_path=None):
        ...
        return os.path.join(
            base_path,
            "{}-{}".format(
                constants.CLI_NAME, hashlib.md5(self.client_connect_address.encode("utf-8")).hexdigest()[0:8]
            ),
        )
